...
class PieChartRenderer(
  ...
  private fun calculateMinimumRadiusForSpacedSlice(
    ...
    val basePointsDistance =
        sqrt((arcEndPointX - arcStartPointX).pow(2f) + (arcEndPointY - arcStartPointY).pow(2f))
    ...
    spacedRadius -=
        sqrt(
            (arcMidPointX - (arcEndPointX + arcStartPointX) / 2f).pow(2f) +
                (arcMidPointY - (arcEndPointY + arcStartPointY) / 2f).pow(2f))
    return spacedRadius
  }
  ...
  private fun getSliceSpace(dataSet: IPieDataSet): Float {
    ...
    val minValueRatio: Float = dataSet.yMin / (mChart.data?.yValueSum ?: 0f) * 2
  ...
  private fun drawDataSet(c: Canvas?, dataSet: IPieDataSet) {
    ...
    for (j in 0 until entryCount) {
      ...
      mPathBuffer.close()
      mBitmapCanvas?.drawPath(mPathBuffer, paintRender)
      angle += sliceAngle * phaseX
    }
  ...
  override fun drawValues(c: Canvas) {
    ...
    for (i in dataSets.indices) {
      ...
      val lineHeight = (calcTextHeight(paintValues, "Q") + convertDpToPixel(4f))
      ...
      for (j in 0 until entryCount) {
        ...
        if (drawXInside || drawYInside) {
          ...
          if (drawXInside && drawYInside) {
            ...
            drawValue(
                c, formatter, value, entry, 0, x, y + lineHeight / 2f, dataSet.getValueTextColor(j))
          }
        }
        if (entry.icon != null && dataSet.isDrawIconsEnabled) {
          ...
          y += iconsOffset.x
          drawImage(c, icon, x.toInt(), y.toInt(), icon.intrinsicWidth, icon.intrinsicHeight)
        }
        xIndex++
      }
      ...
      if (centerText != mCenterTextLastValue || boundingRect != mCenterTextLastBounds) {
        ...
        mCenterTextLayout =
            StaticLayout(
                ...
                centerText.length,
                paintCenterText,
                Math.max(Math.ceil(width.toDouble()), 1.0).toInt(),
  ...
  override fun drawHighlighted(c: Canvas, indices: Array<Highlight?>) {
    ...
    for (i in indices.indices) {
      ...
      val set = indices[i]?.dataSetIndex?.let { mChart.data?.getDataSetByIndex(it) }
  ...
  private fun drawRoundedSlices(c: Canvas) {
    ...
    val dataSet = mChart.data?.dataSet ?: return
    ...
    for (j in 0 until dataSet.entryCount) {
      ...
      if (abs(e.y) > Utils.FLOAT_EPSILON) {
        val x =
            (r - circleRadius) *
                cos(Math.toRadians(((angle + sliceAngle) * phaseY).toDouble()) + center.x)
        val y =
            (r - circleRadius) * sin(Math.toRadians(((angle + sliceAngle) * phaseY).toDouble())) +
                center.y

        paintRender.color = dataSet.getColor(j)
        mBitmapCanvas?.drawCircle(x.toFloat(), y.toFloat(), circleRadius, paintRender)
      }
      angle += sliceAngle * phaseX
    }
    MPPointF.recycleInstance(center)
  ...
  init {
    ...
    paintValues.textSize = convertDpToPixel(13f)
    ...
    paintEntryLabels =
        Paint(Paint.ANTI_ALIAS_FLAG).apply {
          ...
          textSize = convertDpToPixel(13f)
        }
    mValueLinePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.STROKE }
  }
}
