 ...
 */
abstract class ChartData<T : IDataSet<E>, E : Entry> {
  /**
  ...
  var xMin = Float.MAX_VALUE
    protected set
  @JvmField protected var mLeftAxisMax = -Float.MAX_VALUE
  @JvmField protected var mLeftAxisMin = Float.MAX_VALUE
  @JvmField protected var mRightAxisMax = -Float.MAX_VALUE
  @JvmField protected var mRightAxisMin = Float.MAX_VALUE

  /** array that holds all DataSets the ChartData object represents */
  @JvmField protected var mDataSets = mutableListOf<T>()
  ...
  open fun getEntryForHighlight(highlight: Highlight): Entry? {
    return if (highlight.dataSetIndex >= mDataSets.size) null
    else {
      mDataSets[highlight.dataSetIndex].getEntryForXValue(highlight.x, highlight.y)
    }
  }
  ...
  open fun removeEntry(xValue: Float, dataSetIndex: Int): Boolean {
    ...
    val e = dataSet.getEntryForXValue(xValue, Float.NaN)
  ...
  val maxEntryCountSet: T?
    get() {
      if (mDataSets.isEmpty()) return null
      var max = mDataSets[0]
      for (set in mDataSets) {
