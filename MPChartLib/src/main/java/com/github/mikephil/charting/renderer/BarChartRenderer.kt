...
open class BarChartRenderer(
  ...
  @JvmField protected var mBarBuffers: Array<BarBuffer?> = emptyArray()
  ...
  protected open fun drawDataSet(c: Canvas, dataSet: IBarDataSet, index: Int) {
    ...
    val isCustomFill = dataSet.fills != null && dataSet.fills?.isNotEmpty() == true
  ...
  override fun drawValues(c: Canvas) {
    ...
    if (isDrawingValuesAllowed(mChart)) {
      ...
      for (i in 0 until mChart.barData.dataSetCount) {
        ...
        // the value
        val valueTextHeight = calcTextHeight(paintValues, "8").toFloat()
        ...
        if (!dataSet.isStacked) {
          ...
          while (j < buffer!!.buffer.size * mAnimator.phaseX) {
            ...
            if (entry.icon != null && dataSet.isDrawIconsEnabled) {
              ...
              px += iconsOffset.x
              py += iconsOffset.y
              drawImage(c, icon, px.toInt(), py.toInt(), icon.intrinsicWidth, icon.intrinsicHeight)
            }
            j += 4
          }
    ...
    for (high in indices) {
      ...
      if (isStack) {
        if (mChart.isHighlightFullBarEnabled) {
          ...
          val range = e.ranges?.get(high.stackIndex) ?: continue
          y1 = range.from
          y2 = range.to
        }
      } else {
      ...
      c.drawRect(mBarRect, paintHighlight)
    }
  }
