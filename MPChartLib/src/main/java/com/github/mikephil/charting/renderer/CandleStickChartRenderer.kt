  ...
  protected fun drawDataSet(c: Canvas, dataSet: ICandleDataSet) {
    ...
    for (j in mXBounds.min..mXBounds.range + mXBounds.min) {
      ...
      if (showCandleBar) {
        ...
        c.drawLines(mShadowBuffers, paintRender)
        ...
        if (open > close) { // decreasing
          ...
          c.drawRect(
              mBodyBuffers[0], mBodyBuffers[3], mBodyBuffers[2], mBodyBuffers[1], paintRender)
        } else if (open < close) {
          ...
          c.drawRect(
              mBodyBuffers[0], mBodyBuffers[1], mBodyBuffers[2], mBodyBuffers[3], paintRender)
        } else { // equal values
        ...
        c.drawLine(
            mRangeBuffers[0], mRangeBuffers[1], mRangeBuffers[2], mRangeBuffers[3], paintRender)
        c.drawLine(mOpenBuffers[0], mOpenBuffers[1], mOpenBuffers[2], mOpenBuffers[3], paintRender)
        c.drawLine(
            mCloseBuffers[0], mCloseBuffers[1], mCloseBuffers[2], mCloseBuffers[3], paintRender)
      }
    }
  }
