...
class RadarChartRenderer(
  ...
  private fun drawDataSet(c: Canvas, dataSet: IRadarDataSet, mostEntries: Int) {
    ...
    val center = mChart.centerOffsets ?: return
  ...
  override fun drawValues(c: Canvas) {
    ...
    val dataSetCount = mChart.data?.dataSetCount ?: 0
    for (i in 0 until dataSetCount) {
      val dataSet = mChart.data?.getDataSetByIndex(i)
      if (dataSet == null || !shouldDrawValues(dataSet)) continue
  ...
  protected fun drawWeb(c: Canvas) {
    ...
    while (i < maxEntryCount) {
      getPosition(center, mChart.yRange * factor, sliceangle * i + rotationangle, p)
    ...
    val entryCount = mChart.data?.entryCount ?: 0
    for (j in 0 until labelCount) {
      for (i in 0 until entryCount) {
        val r: Float = (mChart.yAxis.mEntries[j] - mChart.yChartMin) * factor
  ...
  override fun drawHighlighted(c: Canvas, indices: Array<Highlight?>) {
    ...
    val radarData = mChart.data
    for (high in indices) {
      val set = radarData?.getDataSetByIndex(high?.dataSetIndex ?: continue)
      if (set == null || !set.isHighlightEnabled) continue
      val e = set.getEntryForIndex(high?.x?.toInt() ?: continue)
