...
class LineChartRenderer(
  ...
  override fun drawData(c: Canvas) {
    ...
    c.drawBitmap(drawBitmap, 0f, 0f, paintRender)
  }
  ...
  protected fun drawHorizontalBezier(dataSet: ILineDataSet) {
    ...
    mBitmapCanvas!!.drawPath(cubicPath, paintRender)
    paintRender.pathEffect = null
  }
  ...
  protected fun drawLinear(c: Canvas?, dataSet: ILineDataSet) {
    ...
    if (dataSet.colors.size > 1) {
      ...
      for (j in mXBounds.min until max) {
        ...
        canvas!!.drawLines(mLineBuffer, 0, pointsPerEntryPair * 2, paintRender)
      }
      ...
      if (e1 != null) {
        ...
        if (j > 0) {
          ...
          paintRender.color = dataSet.color
          canvas!!.drawLines(mLineBuffer, 0, size, paintRender)
        }
      }
    }
    paintRender.pathEffect = null
  ...
  private inner class DataSetImageCache {
    ...
    private var circleBitmaps: Array<Bitmap?> = emptyArray()
    ...
    fun fill(set: ILineDataSet, drawCircleHole: Boolean, drawTransparentCircleHole: Boolean) {
      ...
      for (i in 0 until colorCount) {
        ...
        val circleBitmap = Bitmap.createBitmap(
          (circleRadius * 2.1).toInt(), (circleRadius * 2.1).toInt(), conf
        )
