...
class ScatterChartRenderer(
  ...
  private fun drawDataSet(c: Canvas?, dataSet: IScatterDataSet) {
    ...
    val max = min(ceil(dataSet.entryCount * mAnimator.phaseX).toInt(), dataSet.entryCount)
    for (i in 0 until max) {
      ...
      renderer.renderShape(
          c, dataSet, mViewPortHandler, mPixelBuffer[0], mPixelBuffer[1], paintRender
      )
    }
  }
  ...
  override fun drawValues(c: Canvas) {
    ...
    if (isDrawingValuesAllowed(mChart)) {
      ...
      for (i in 0 until mChart.scatterData.dataSetCount) {
        ...
        val shapeSize = convertDpToPixel(dataSet.scatterShapeSize)
