...
class Legend() : ComponentBase() {
  ...
  var extraEntries: Array<LegendEntry> = arrayOf()
    private set
  /** @return true if a custom legend entries has been set default false (automatic legend) */
  /**
   * Are the legend labels/colors a custom value or auto calculated? If false, then it's auto, if
   * true, then custom. default false (automatic legend)
   */
   ...
   */
  /**
   * the space between the legend entries on a vertical axis, default 2f private float mYEntrySpace
   * = 2f; / ** the space between the form and the actual label/text
   */
   ...
   * the chart, then this affects the width of the legend. / If the legend is to the top/bottom of
   * the chart, then this affects the height of the legend. / If the legend is the center of the
   ...
   * The maximum relative size out of the whole chart view. / If the legend is to the right/left of
   * the chart, then this affects the width of the legend. / If the legend is to the top/bottom of
   * the chart, then this affects the height of the legend. / default: 0.95f (95%)
   *
   * @param maxSize
   */
  ...
  fun setExtra(colors: IntArray, labels: Array<String?>) {
    ...
    for (i in 0 until colors.size.coerceAtMost(labels.size)) {
      val form =
          when (colors[i]) {
            0, ColorTemplate.COLOR_SKIP -> LegendForm.NONE
            ColorTemplate.COLOR_NONE -> LegendForm.EMPTY
            else -> LegendForm.DEFAULT
          }
      val entry = LegendEntry(formColor = colors[i], label = labels[i], form = form)
      entries.add(entry)
    }
    extraEntries = entries.toTypedArray()
  }
   ...
   * * A null label will start a group. This will disable the feature that automatically calculates
   * the legend entries from the datasets. Call resetCustom() to re-enable automatic calculation
  ...
  @JvmField var mNeededWidth = 0f

  /** the total height of the legend (needed height space) */
  @JvmField var mNeededHeight = 0f
  @JvmField var mTextHeightMax = 0f
  var mTextWidthMax = 0f
  /**
   * If this is set, then word wrapping the legend is enabled. This means the legend will not be cut
   * off if too long.
   *
   * @return
   */
  /**
   * Should the legend word wrap? / this is currently supported only for: BelowChartLeft,
   * BelowChartRight, BelowChartCenter. / note that word wrapping a legend takes a toll on
   * performance. / you may want to set maxSizePercent when word wrapping, to set the point where
   * the text wraps. / default: false
   *
   * @param enabled
   */
  ...
  fun calculateDimensions(labelpaint: Paint, viewPortHandler: ViewPortHandler) {
    ...
    when (orientation) {
      ...
      LegendOrientation.HORIZONTAL -> {
        ...
        mNeededHeight =
            (labelLineHeight * mCalculatedLineSizes.size.toFloat() +
                labelLineSpacing *
                    (if (mCalculatedLineSizes.size == 0) 0 else mCalculatedLineSizes.size - 1)
                        .toFloat())
      }
    }
