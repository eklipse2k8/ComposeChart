...
class LegendRenderer(
  ...
  private var computedEntries: MutableList<LegendEntry> = ArrayList(16)
  ...
  fun renderLegend(c: Canvas) {
    ...
    labelPaint.textSize = mLegend.textSize
    labelPaint.color = mLegend.textColor
    val labelLineHeight = getLineHeight(labelPaint, legendFontMetrics)
    val labelLineSpacing =
        (getLineSpacing(labelPaint, legendFontMetrics) + convertDpToPixel(mLegend.yEntrySpace))
    val formYOffset = labelLineHeight - calcTextHeight(labelPaint, "ABC") / 2f
      ...
      LegendOrientation.HORIZONTAL -> {
        ...
        while (i < count) {
          ...
          val formSize =
              ...
              else convertDpToPixel(e.formSize)
      ...
      LegendOrientation.VERTICAL -> {
        ...
        while (i < entries.size) {
          ...
          if (e.label != null) {
            ...
            if (direction === LegendDirection.RIGHT_TO_LEFT)
                posX -= calcTextWidth(labelPaint, e.label).toFloat()
            if (!wasStacked) {
              drawLabel(c, posX, posY + labelLineHeight, e.label)
            } else {
              posY += labelLineHeight + labelLineSpacing
   ...
   */
  protected fun drawForm(c: Canvas, x: Float, y: Float, entry: LegendEntry, legend: Legend) {
