...
/**
 * Class that holds predefined color integer arrays (e.g. ColorTemplate.VORDIPLOM_COLORS) and
 * convenience methods for loading colors from resources.
 *
...
object ColorTemplate {
  ...
  /**
   * THE COLOR THEMES ARE PREDEFINED (predefined color integer arrays), FEEL FREE TO CREATE YOUR OWN
   * WITH AS MANY DIFFERENT COLORS AS YOU WANT
   */
  @JvmField
  val LIBERTY_COLORS =
      intArrayOf(
  ...
  /**
   * turn an array of resource-colors (contains resource-id integers) into an array list of actual
   * color integers
   *
   * @param r
   ...
   * @return
   */
  fun createColors(r: Resources, colors: IntArray): List<Int> = colors.map { r.getColor(it) }
   ...
   * @return
   */
  fun createColors(colors: IntArray): List<Int> = colors.toList()
}
