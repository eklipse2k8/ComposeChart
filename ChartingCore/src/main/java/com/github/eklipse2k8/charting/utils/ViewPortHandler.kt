...
class ViewPortHandler {
   ...
   * @param matrix
   */
  private fun limitTransAndScale(matrix: Matrix, content: RectF?) {
  ...
  fun setMaximumScaleX(xScale: Float) {
    var value = xScale
    if (value == 0f) value = Float.MAX_VALUE
    maxScaleX = value
    limitTransAndScale(matrixTouch, contentRect)
  }
  ...
  fun setMinMaxScaleX(minScaleX: Float, maxScaleX: Float) {
    var minValueX = minScaleX
    var maxValueX = maxScaleX
    if (minValueX < 1f) minValueX = 1f
    if (maxValueX == 0f) maxValueX = Float.MAX_VALUE
    this.minScaleX = minValueX
    this.maxScaleX = maxValueX
    limitTransAndScale(matrixTouch, contentRect)
  }
